	...
	for i,a in enumerate(arr_arr):
		...
		if np.isfinite(a).all():
			...
			if i < len(farr_arr)-2 and np.isfinite(farr_arr[i+2]).all() and np.isfinite(farr_arr[i+1]).all():
				# for larger positive values of yhat, check whether this corresponds with median
				#RmOffset =( 1 - np.exp( 1 - np.clip(np.divide(np.clip(yhat,0,1) ,np.clip(farr_arr[i+1] - farr_arr[i],0.01,1)),1,1000))) * R
				RmOffset = np.clip(farr_arr[i+2] - farr_arr[i+1] - yhat,0,1) * R * 0.25
				print "Computed RmOffset"
			#Rm = np.clip(Rm+RmOffset,Rmin,R)
			#Rm += RmOffset
